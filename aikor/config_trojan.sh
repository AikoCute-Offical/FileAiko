...
read -p "Vui lòng chọn config cấu hình: " domain_trojan_aiko
    ...
    wget https://raw.githubusercontent.com/AikoCute/Aiko-Config/aiko/Config-Trojan.yml -O /etc/AikoR/aiko.yml
    sed -i "s/NodeID:.*/NodeID: ${id_trojan_aiko}/g" /etc/AikoR/aiko.yml
    sed -i "s/CertDomain:.*/CertDomain: ${domain_trojan_aiko}/g" /etc/AikoR/aiko.yml
